...
하지만 웹 컨테이너가 서블릿이 한번에 하나의 요청만 처리한다는 걸 어떤 식으로 보장한다는 말이죠?
웹 컨테이너를 만드는 회사는 다음 둘 중 하나를 선택할 수 있습니다.
첫번째로 컨테이너가 서블릿 하나를 만듭니다. 그런 다음 서블릿이 요청을 하나씩 처리하는 거죠. 즉 하나의 요청을 완벽하게 끝낸 다음에야 다음 요청을 받는 방식 말입니다. 둘째로 컨테이너가 서블릿 인스턴스 풀을 만드는 겁니다. 요청이 동시에 여럿 들어오면 각각의 서블릿 인스턴스가 요청을 하나씩 처리하는 방식입니다